...
static const char *conf_prefix = NULL;
...
static void exit_usage (int exit_status) /* {{{ */
{
  fprintf ((exit_status == EXIT_FAILURE) ? stderr : stdout,
      ...
      "                   (Default: "DEF_NODE")\n"
      "    -n <dest>      Namespace prefix for the metrics (defaults to none).\n"
...
static int send_random_event (int sock, unsigned short seed[static 3]) /* {{{ */
{
  ...
  buffer_size = snprintf (buffer, sizeof (buffer), "%s%06li:%li|%s",
                          conf_prefix, rnd, value, type);
